    ...
    <div class={mergeClasses("flex flex-col gap-2", properties.class)}>
      <For each={properties.options}>
        {(option) => {
          ...
          return (
            <label class={mergeClasses("flex items-start gap-2.5 py-1", isDisabled() ? "cursor-not-allowed opacity-60" : "cursor-pointer")}>
              <span class={TOGGLE_INDICATOR_WRAPPER_CLASS}>
                ...
                <Show when={properties.selectionMode === "multiple"}>
                  <svg class="pointer-events-none absolute top-1/2 left-1/2 h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 text-white opacity-0 peer-checked:opacity-100" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor">
                    <path d="m382-354 339-339q12-12 28-12t28 12q12 12 12 28.5T777-636L410-268q-12 12-28 12t-28-12L182-440q-12-12-11.5-28.5T183-497q12-12 28.5-12t28.5 12l142 143Z" />
                  </svg>
                </Show>
                <Show when={properties.selectionMode === "single"}>
                  <span class="pointer-events-none absolute top-1/2 left-1/2 h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white opacity-0 peer-checked:opacity-100" />
                </Show>
              </span>
